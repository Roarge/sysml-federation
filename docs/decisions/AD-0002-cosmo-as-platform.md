...
exception. The router module, wgc 0.130.1 and `@wundergraph/composition`
0.63.3 all declare Apache-2.0 in their package metadata. According to the
vendor, the platform runs without licence fees or feature gates, and "will
...
No graph token is required with a static config, and with the
token empty the default Cosmo Cloud exporters are disabled. Two
...
Production" at every start without a token. Since router 0.215.0 an
...
`COSMO_TELEMETRY_DISABLED=true`. The README's "no network dependency
at all" is true only with those set, so the public wording carries both
caveats rather than going around them.
...
needs Node and happens outside the build with its output committed. The
router's Go module has only commit pseudo-versions and has broken its API
twice, so the router is a copied binary driven by configuration rather than
a library. Cosmo's subgraphs can now be compiled to protobuf and served
over gRPC with GraphQL kept as the schema language, and that is the
README's deeper reason for Cosmo. The README reads this as an admission
that the valuable part was never the wire format.

## Decision

We will use WunderGraph Cosmo as the federation platform, pinned to router
0.343.1 and wgc 0.130.1 and bumped together. It will run from a static
execution configuration with no control plane, no graph token and telemetry
switched off by `DO_NOT_TRACK=1`, `COSMO_TELEMETRY_DISABLED=true`,
`TRACING_ENABLED=false` and `METRICS_OTLP_ENABLED=false` baked into the
image. The router binary is copied out of the vendor's image with its
Apache-2.0 licence text beside it and started as a child process.
...
from. All three subgraphs stay standard GraphQL subgraphs so that
subscriptions work.
...
Cosmo's gRPC services and its extension-module subgraph route, which the
research listed as its fourth and fifth options. Both keep the GraphQL
SDL as the contract and are the direction the README admires, and both
explicitly do not support subscriptions, so the two-app live update
would fall back to polling. The research keeps the extension-module route
for a later second example.
...
production" and says what the demo does about it. The demo's answer is that
no control plane exists to fetch from and the configuration is committed
and tested for drift. The README's air-gap sentence gains the qualification
and names the image's variables. CI has no Node, so the committed
...
exceeds its threshold, and every router bump is a rebuild of the demo
image. The router binary is about 40 MB compressed and sets the floor of
SR-06's budget. The licence text travels with the binary (SR-07) and the
repository carries a `NOTICE` (SR-08).

Four spikes belong to this decision. The first runs the image under
...
demonstration. The second composes the three draft schemas with wgc and
runs gqlgen over the nested `@requires`, which the research never ran. In
the third, the supervisor starts the copied binary with environment alone
in a distroless image with no YAML file present. The fourth observes what
`/health/ready` reports with the subgraph ports closed.

## Requirements affected
SR-03, SR-07, SR-42, SR-43

## Sources
The repository README, "Why Cosmo, and not simply GraphQL". The LICENSE file at the root of wundergraph/cosmo and the package metadata of router 0.343.1, wgc 0.130.1 and `@wundergraph/composition` 0.63.3. The vendor's pages on `wgc router compose` and on running the router from a static execution configuration, and `router/.env.example` for the telemetry variables. The commit that removed `composition-go`. [What the research overturned](../articles/03-what-the-research-overturned.md) gives the readings this record rests on and marks the ones that stayed unverified.
