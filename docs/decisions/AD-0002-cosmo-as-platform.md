...
0.63.3 all declare Apache-2.0 in their package metadata. The vendor
...
needs Node and happens outside the build with its output
committed. The router's Go module has only commit
pseudo-versions and has broken its API twice, so the router is a copied
binary driven by configuration rather than a library. The
...
from, and all three subgraphs stay standard GraphQL subgraphs so that
subscriptions work.
...
for drift, and the README's air-gap sentence gains the qualification
...
exceeds its threshold, and every router bump is a rebuild of the
demo image.
The router binary is about 40 MB compressed and sets the floor of SR-06's
budget. The licence text travels with the binary (SR-07) and the
...
demonstration. The second composes the three draft schemas with wgc
and runs gqlgen over the nested `@requires`, which the research never ran.
The third starts the copied binary from the supervisor with
environment alone in a distroless image with no YAML file present. The
fourth observes what `/health/ready` reports with the subgraph
ports closed.
