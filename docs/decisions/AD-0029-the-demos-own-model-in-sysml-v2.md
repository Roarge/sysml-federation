...
make targets, the workflows and every live check are actions or cases of their
own. The published boards correspond to named views. The identifiers of the
light scheme are the model's short names. A unit test in `internal/trace` fails
when the model and the repository disagree on an identifier, a test name, a
check file, a published image, a check's inventory or a compose service.
