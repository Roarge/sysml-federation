...
repository expose projects, commits and elements the same way. For this
project the README values the second more, since it is what gives the model
an interface that is not a vendor's. It then states under "What this is
not" that the adapter reads files rather than fronting a repository and is
not a SysML v2 API implementation. Under "Placeholders" it says there is no
model repository behind the adapter, so versioning is stood in for rather
than taken from a conforming repository's commits.
...
and one `docker run` with nothing but Docker installed, and the brief's
non-goals repeat that the demo is not a SysML v2 API implementation. With a
Java 17 tool, the pilot can also emit the standard JSON serialisation from
text files. The serialisation is a flat element graph in which memberships
are themselves elements and every reference is a UUID whose stability
across runs is unverified. That is the shape a repository returns over the
API, and the research keeps it as the best second input path.
...
language. At the API level, `elementId` is a tool-assigned UUID, and the
API 1.0 OpenAPI has no human id field. Every published element therefore
carries a short name, and the adapter falls back to the qualified name.
State is held in memory with a version counter as the versioning stand-in.
...
it, since a subset parser cannot prove conformance. Which parser reads the
files is AD-0015, and what the projection does with elements nobody has
projected is AD-0005.

## Decision

We will have the adapter read the model from SysML v2 textual notation
files on disk when it starts and hold the parsed model in memory. A
monotonically increasing counter served as `Model.version` will stand in
for a repository's commits. The adapter neither implements nor calls the
SysML v2 API. The example ships its model file inside the image, and the
parser records the source span of every numeric literal so the served text
can be patched in place. Before it becomes a fixture, the example file is
accepted by the OMG pilot 2026-07 and the OpenSysML command line (SR-45).
...
UUIDs and shape it after the API's identity fields. That way a later swap
to a repository is a source change rather than a schema change.
...
for three reasons. It needs Java 17 at build time, the adapter would have
to walk the metamodel it was meant to hide, and the `elementId` values are
UUIDs whose stability across regenerations could not be verified.
...
rests on spikes that belong to the parser. Those spikes quote ports and
`connect` from the OMG training folders, confirm the plain numeric binding
and the duration literal in the pilot, and read the 2.1 Beta 2 change list
before the validation run. The validation itself runs locally and never in
CI.
