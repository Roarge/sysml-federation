...
responds at once, and D1 chose a viewer whose numbers are inline inputs.
The brief fixes the editable set as exactly the five server throughput
values and the limit of PIPE-R1, and its assumptions state the rule behind
that. A literal in the source is editable and a bound expression is not,
the apps expose controls only for the editable set, and edits land in the
served model text and its version counter and never on disk. The derived
limits are bound by expressions in the model so that they follow PIPE-R1
(P14), which is what makes them read-only. The non-goals rule out
persistence across restarts, authentication and multi-user editing.

The mechanism comes from three decisions. P1 sends every read and write
through the router, so a value edit is a mutation the router plans to the
adapter and no app talks to a subgraph directly. P10 keeps state in memory
with a reset and patches edited literals into the served text. D8's parser
records source spans, which is what makes the patch possible. C-93 puts
these together. `setAttribute`, `setLimit` and `resetModel` patch the
...
in the container, and a restart restores the shipped values. C-95 records
the remaining non-goals as a constraint. The research critique had asked
whether a mutation rewrites the `.sysml` text on disk, which would need a
lossless syntax tree with trivia, or mutates in-memory state lost when the
container exits, and P10 is the answer. The gate 1 review added that
invalid input has to be survived, and SR-25's rule is that a submitted
...
other request (P1), and apply an accepted edit by replacing the literal at
its recorded source span in the in-memory model text, re-parsing,
rebuilding the projection and incrementing the version in one step (P10,
C-93). The adapter accepts any value that is a literal in the source and
...
The critique raised it and the brief rules it out. Persistence across
...
plan rejected it under P1 with the sentence that no UI talks to a subgraph
...
authentication, no multi-user editing and no second machine (C-95), and
every edit is lost when the container stops. The constraints list names no
spike for this decision.

## Requirements affected
SR-09, SR-13, SR-22, SR-24, SR-25, SR-38, SR-44

## Sources
README "Placeholders" and "The pipeline example", the design brief D1, P1, P10, P14, the editable set, assumptions and non-goals, the constraints list C-93, C-95, the requirements list SR-09, SR-13, SR-22, SR-24, SR-25, SR-38, SR-39, SR-40, SR-41, SR-44, the research critique (edit semantics and persistence), the engineering log's gate 1 entry (invalid input criteria), the architecture description V2 mutations and V5 patch.
