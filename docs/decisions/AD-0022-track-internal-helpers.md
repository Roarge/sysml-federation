...
parameters and go-cmp, so a comparison between mismatched types is a compile
error at the call site rather than a report at run time. The second,
...
and tracked tests import them. The mechanism is three additions. `.gitignore`
gains `!/internal/` with `!/internal/**/`, and the Makefile's `ALLOWLIST_ROOTS`
gains `internal` so that a forgotten file under `internal/` is reported. The
tracking policy is amended so that `internal/` moves from the local-only column
to the tracked one. The two allowlist rules open the whole `internal/` tree to
the allowlisted extensions, and these two packages are all it holds.
...
tracking policy would stand as written. The cost is the same loop and the same
comparison repeated in every table-driven test. It also includes two packages
left on disk with no consumer, which is the opposite of what the rule directing
...
repository in an afternoon. They therefore count towards the impression the
code makes, though not towards SC-06's figures, which name no component under
`internal/`.

The tracking policy loses its clean shape. `internal/` was one line in the
local-only column and becomes a tracked tree with an exception noted beside it.
Any future helper that exists only to constrain how the code is written has to
live outside the tracked tree instead. The `assert` package's
