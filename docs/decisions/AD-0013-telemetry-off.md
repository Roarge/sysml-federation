...
## Sources
The vendor's `router/.env.example`, which is the one place `DO_NOT_TRACK` and `COSMO_TELEMETRY_DISABLED` are named, and the vendor's pages on tracing and metrics exporters and on running without a graph token. [What the research overturned](../articles/03-what-the-research-overturned.md) for the finding as it stands and for what remains inferred rather than run, and [The image](https://github.com/Roarge/sysml-federation/blob/main/examples/pipeline/README.md#the-image) in the example's README for the five `ENV` lines, and its verification record for the air-gap demonstration.
