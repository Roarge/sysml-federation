...
The research set out to confirm that no Go parser for the notation existed, and
that was refuted. Three exist. Open-MBEE/OpenSysML is Apache-2.0, a
hand-written recursive-descent parser with semantic resolution, an LSP, a REPL
and a gRPC server. It was young and at v0.2.1 when it was read, with all its
library code under `internal/`. The second, mycr0ft/gosysml, is MIT and
ANTLR-based, new at the time of reading and with a grammar descended from a
2023 release. The third, dVoo/gosysml2, is GPL-3.0. Beside them sits the
antlr/grammars-v4 sysml-v2 grammar, MIT, which would generate a very large
parser from a KEBNF that two downstream projects document as ambiguous. So does
nomograph-ai/tree-sitter-sysml, MIT, whose Go binding is cgo and would end the
`CGO_ENABLED=0` cross-compilation the image depends on. Syside is closed source
and licence-gated, with air-gapped use needing a Business plan.
...
conforming repository returns over the API. That route comes at the cost of a
JVM at build time and a flat element graph in which even memberships are
elements. OpenSysML's gRPC server could run as a sidecar and return its own
`SymbolInfo` tree, which is not the OMG JSON and whose API is pre-1.0.
...
node carries its byte range in the source. It accepts exactly the constructs of
a listed subset, with ports and connections filled in by a spike, and refuses
anything else with the file, line and column of the first offending token. The
parser has no opinion about meaning. Name resolution, expression evaluation and
the reading of `require constraint` belong to the model package above it. The
example model is validated separately with the OMG pilot 2026-07 and the
OpenSysML command line before it becomes a fixture (SR-45). OpenSysML becomes
the replacement when its maintainers expose a public Go package or freeze the
gRPC contract.

## Alternatives considered

Consuming the OMG JSON serialisation produced by the pilot, checked into the
example or produced in a build stage. It needs Java 17 at build time, and its
output is a flat element graph the adapter would have to walk in the very
metamodel it exists to hide. Its `elementId` values are tool-assigned UUIDs
whose stability across runs is unverified. The research keeps it as the best
second input path, because its shape is what a real repository returns.
...
today: OpenSysML's code is under `internal/`, gosysml was new with a grammar
from 2023, gosysml2 is GPL-3.0 and tree-sitter needs cgo. A parser generated
from grammars-v4 would be huge and inherit the grammar's documented
...
refused, never skipped, which is what SR-18 requires. Refusal is also the
smaller and more honest first cut of the escape hatch the README leaves open:
nothing is served generically in this phase (AD-0005).
...
numeric binding and the duration literal. Before the example is written, the
ports and `connect` syntax is quoted from OMG training folders 09 and 10. The
2.1 Beta 2 change list is read before the validation run, since the pilot
implements 2.1 Beta 2 and the adapter claims 2.0 formal.
