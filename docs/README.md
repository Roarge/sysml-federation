...
The goal of this project is to make SysML v2 models easier to integrate with the domain models of the organisation's other tools, finally enabling the promise that model-based systems engineering has been five years away from for the past 25 years. The proposal is to adopt federation. The model's owners publish a small projection of it, and other services attach their own data to its objects by agreeing on an identifier.
