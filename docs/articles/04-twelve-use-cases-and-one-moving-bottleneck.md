...
The design brief opens by quoting the README: "This repository is an argument that federation is the missing integration layer for open MBSE." It then says what the demo has to do about that claim, in words that every later document was measured against:
...
Before any board existed, the README, the plan, the brief and the use cases were read against each other in one pass. Every arithmetic claim held and the writing rules were met. What the pass found were gaps in specification rather than errors.
...
Two suggestions were declined. Quoting the README's argument at length was refused because the brief quotes one sentence and leaves the rest to the README itself, and trimming each app's description to a single sentence because short paragraphs read better.
