...
Three parts of the demo are stand-ins. The adapter reads [a directory of files](../decisions/AD-0003-adapter-reads-files.md) where a real deployment would front a SysML v2 repository. Editing through the projection is [scaffolding](../decisions/AD-0004-editing-as-scaffolding.md) that contradicts the read-path rule above, and a real deployment would write through the SysML v2 API. The capacity arithmetic is [idealised](../decisions/AD-0006-idealised-capacity-model.md), and nobody should plan capacity with it.
