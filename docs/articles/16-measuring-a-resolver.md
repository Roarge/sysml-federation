...
> **Gold set**
>
> The links a resolver's output is measured against, each one checked by people. Traceability research calls it an answer set. It includes the inputs that should get no link as well as those that should, and it holds only for the version of the system it was built from.
...
A score against last month's gold set describes last month's system. For a system that changes daily, I'd measure at every commit, against the gold set as it stood at that commit. The trend matters more than any single figure. Defect prediction research tests its machine learning models for predicting defects the same way. [Bangash and colleagues](https://arxiv.org/abs/1911.06348) evaluate so that "models are trained only on the past, and evaluations are executed only on the future". They also warn that broad claims "might be contradicted by the next upcoming release". For trace links, [Rahimi and Cleland-Huang](https://link.springer.com/article/10.1007/s10664-017-9561-x) start from "a tendency for trace links to degrade over time as the system continually evolves". They then evaluated a tool that evolves links across 27 releases of the Cassandra database.
