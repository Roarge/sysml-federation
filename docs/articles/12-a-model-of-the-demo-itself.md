...
Thirty checks and monitors sit in five groups, and the three groups that edit the demo's one in-memory state run one check at a time from one location with no retries, because a retried or a parallel run would edit it twice. Without credentials `docker run` runs the demo exactly as before, and nothing under `checkly/` is built, run or read.
