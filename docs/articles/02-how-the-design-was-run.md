...
A few house rules shaped the code from the first commit. The gitignore is an allowlist, so nothing gets committed unless a rule names it, which is why [tracking two test helper packages](../decisions/AD-0022-track-internal-helpers.md) needed a decision of its own. Hand-written Go may not use an empty interface as the type of a value, because a value of that type can hold anything and the compiler can no longer check it. [Generated code](../decisions/AD-0016-generated-code-exempt.md) is exempt. Tests come first, and commits reach `main` only through a pull request.
