...
Part 12 of 13 in [Federating a systems model](../README.md).
...
The same-origin protection on the subscription socket was proved by hand and never by a browser. A handshake sent with a foreign origin came back `403 Forbidden` from both services that serve subscriptions, which is the check firing, and that is the whole of the evidence for it. No page served from anywhere else has been made to open the connection, which is how a real attempt would arrive.
