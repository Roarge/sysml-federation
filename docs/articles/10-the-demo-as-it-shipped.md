...
`adapter/model` builds the projected model from that tree, evaluates expressions with units and a cycle guard, reads the quantity, comparison and limit from each constraint, and patches a literal in the source text for `setAttribute` and `setLimit`, producing a new immutable model with a new version each time. A second model, a warehouse, is the fixture that proves the same code paths carry a model sharing none of the example's vocabulary. What the two word tests enforce beside it is a named list rather than a general rule. No source under `adapter/`, its generated code included, may contain `'PIPE'`, `PIPE-`, `Pipeline`, `Component`, `capacity`, `QueryInput`, `QueryOutput`, `throughput`, `latency`, `requiredRate`, `ingest`, `indexA` or `indexB`, each a case-sensitive substring and three of them listed in both capitalisations, and the list carries its own exceptions. `parse`, `serve`, `Server`, `Query` and `target` are off it because they are ordinary words of a parser, of the standard library and of the SysML libraries, and the elements that carry them are caught through the names that are on it. The capacity service is walked against a shorter list which does include `Server` and which skips generated code, and `throughput`, the attribute that service reads, is on that list and never appears, because the name reaches the service only through the pair the supervisor hands it. `capacity` is off the list because the service is named after the quantity it computes, and the package clause, a doc comment on its resolver and the field its schema publishes all carry the word, though the string the service compares a requirement's quantity against still comes from that same pair rather than from a literal. Test files are outside both walks, and the warehouse fixture is held to every name on the adapter's list but `capacity`, the one attribute it keeps so the analysis tests can reuse it.
...
Publishing is a GitHub Actions workflow on `v*` tags, the decision on [publishing on tags](../decisions/AD-0020-publish-on-tags.md). It builds for `linux/amd64` and `linux/arm64`, pushes the version tag, and then reads the manifest back. For each platform it sums the compressed layer sizes and the config size and fails the run if the sum exceeds 80,000,000 bytes, the 80 MB budget in the decimal unit the registry uses. Registry-counted sizes cannot be had without pushing, so the measurement happens on the published version tag. A native runner of each architecture then pulls the image by the digest the build reported and the gate measured, without a credential, starts it and checks the paths and answers it serves, and `latest` is moved to the same bytes only once both platforms are under the budget and both runs have passed. A run that fails the gate or either native run leaves `latest` pointing where it pointed. Beyond the unit tests, CI validates the demo's own model and publishes the image on version tags. The first tag was `v0.1.0`. The package was then made public once, by hand, a flip the registry does not allow anyone to undo, and the launch line answered from a host holding no credential for it.
